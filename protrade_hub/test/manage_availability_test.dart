...
void main() {
  testWidgets('ManageAvailabilityWidget adds availability slots',
      (WidgetTester tester) async {
    final mockAvailability = <String, List<Map<String, String>>>{};
      ...
      home: Scaffold(
        body: ManageAvailabilityWidget(
          ...
          onAvailabilityAdded: (day, slot) {
            mockAvailability.putIfAbsent(day, () => []).add(slot);
          },
          onAvailabilityRemoved: (day, slot) {
            mockAvailability[day]?.remove(slot);
          },
          onNewDayAdded: (day) => mockAvailability.putIfAbsent(day, () => []),
        ),
      ),
    ));
    ...
    // Add new day
    await tester.tap(find.byKey(Key('addNewDayButton')));
    await tester.pumpAndSettle();

    expect(mockAvailability.containsKey('Monday'), isTrue);

    // Add slot
    await tester.enterText(find.byKey(Key('timeField')), '09:00 - 12:00');
    await tester.tap(find.byKey(Key('addSlotButton')));
    await tester.pumpAndSettle();

    expect(mockAvailability['Monday']!.first['time'], '09:00 - 12:00');
  });
}
